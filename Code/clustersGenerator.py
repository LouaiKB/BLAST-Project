...
class ClustersGenerator:
    ...
    # static method for the clusterization
    @staticmethod
    def clusterizer(first_id, second_id, third_id, cluster_dict):
        if first_id == second_id: 
            if first_id in cluster_dict.keys():
                # this list contains the hits and the querys from the match 
                resultsFromTheMatch = [first_id, second_id, third_id]
                ...
                for i in range(len(resultsFromTheMatch)):
                    ...
                    for key in cluster_dict.keys():
                        ...
                        if (resultsFromTheMatch[i]) in cluster_dict[key]:
                            ...
                            if i == 0:
                                for z in (resultsFromTheMatch[1], resultsFromTheMatch[2]):
                                    ...
                                    if z not in cluster_dict[key]:
                                        cluster_dict[key] += (z,); counter += 1
                ...
                if counter == 0: 
                    ...
                    cluster_dict[first_id] = (first_id, second_id, third_id)
            ...
            for result1 in values[0]:
                ...
                for result2 in concatenatedListOfValues:
                    ...
                    if result1[0] == result2[0]: 
                        ClustersGenerator.clusterizer(result1[0], result1[1], result2[1], cluster)
                        
                    # if there is a match between querys 
                    if result1[1] == result2[1]:
                        ClustersGenerator.clusterizer(result1[0], result1[1], result2[0], cluster)
